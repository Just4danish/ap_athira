...
export default function GreaseTrapDetailsPage({ navigation, route }) {
  ...
  useEffect(() => {
    if (itemInfo !== null) {
      if (itemInfo.fixture.image !== null) {
        setImage({ uri: baseURL + itemInfo?.fixture.image + `?${new Date()}` })
      }
      else {
        setImage(noImage)
      }
  ...
  if (itemInfo === null){
    ...
    return (
      <ScrollView
        ...
        <Item label="Fixture ID" value={itemInfo.fixture.fixture_id} editEnabled={editEnabled} />
        <Item label="Fixture Description" value={itemInfo.fixture.fixture} editEnabled={editEnabled} />
        <Item label="Quantity" value={itemInfo.qty} editEnabled={editEnabled} />
        <Item label="Remarks" value={itemInfo.remarks} editEnabled={editEnabled} />
        <Item label="Status" value={itemInfo.status} editEnabled={editEnabled} />
        <View
          style={{
            width: '100%',
            height: 200,
